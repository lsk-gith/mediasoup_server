  ...
  : channel(channel), payloadChannel(payloadChannel)
{
	...
	//增加sha 返回参数
	auto sha_all = json::array();
	auto fingerprints = RTC::DtlsTransport::GetLocalFingerprintsNew();
	for (size_t i{ 0 }; i < fingerprints.size(); ++i)
	{
		sha_all.emplace_back(json::value_t::object);
		auto& jsonEntry   = (sha_all)[i];
	...
	MS_DEBUG_TAG(info, "get sha-all[sha-all:%s]", sha_all.dump().c_str());
	this->shared->channelNotifier->Emit(std::to_string(Logger::pid), "running", sha_all);
	...
	switch (request->methodId)
	{
		...
		case Channel::ChannelRequest::MethodId::WORKER_CREATE_WEBRTC_SERVER:
		{
			try
			{
				...
				MS_DEBUG_TAG(info, "sha-all[sha-all:%s]", sha_all.dump().c_str());
				//增加sha 返回参数
				MS_DEBUG_DEV("WebRtcServer created [webRtcServerId:%s]", webRtcServerId.c_str());

				request->Accept(sha_all);
			}
		...
		case Channel::ChannelRequest::MethodId::WORKER_CREATE_ROUTER:
		{
			...
			MS_DEBUG_TAG(info, "routerId[: %s]", routerId.c_str());
			auto* router = new RTC::Router(this->shared, routerId, this);

			this->mapRouters[routerId] = router;

			MS_DEBUG_TAG(info,"Router created [routerId:%s]", routerId.c_str());
