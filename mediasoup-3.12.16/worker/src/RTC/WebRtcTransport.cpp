...
namespace RTC
{
	  ...
	  : RTC::Transport::Transport(shared, id, listener, data)
	{
		...
		auto finger = data.find("finger");
		if (finger != data.end())
		{
			if (!finger->is_string())
			{
				MS_THROW_TYPE_ERROR("wrong finger (not a string)");
			}
		}
		...
		if (password != data.end())
		{
			...
			{
				MS_THROW_TYPE_ERROR("wrong password (not a string)");
			}
		}
		...
		try
		{
			...
			this->iceServer = new RTC::IceServer(
				this, finger->get<std::string>(), password->get<std::string>());
			// Create a DTLS transport.
			this->dtlsTransport = new RTC::DtlsTransport(this);
			MS_DEBUG_TAG(info,"recvIce [finger:%s] [password:%s]", finger->get<std::string>().c_str(), password->get<std::string>().c_str());
			// NOTE: This may throw.
