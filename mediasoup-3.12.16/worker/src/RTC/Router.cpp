	...
	void Router::HandleRequest(Channel::ChannelRequest* request)
	{
		...
		switch (request->methodId)
		{
			...
			case Channel::ChannelRequest::MethodId::ROUTER_CREATE_WEBRTC_TRANSPORT_WITH_SERVER:
			{
				...
				webRtcTransport->FillJson(data);
				MS_DEBUG_TAG(info,
					"WebRtcTransport with WebRtcServer created [transportId:%s] and response:[data%s]", transportId.c_str(), data.dump().c_str());
				request->Accept(data);
				break;
			}
			...
			case Channel::ChannelRequest::MethodId::ROUTER_CREATE_PIPE_TRANSPORT:
			{
				...
				MS_DEBUG_TAG(info, "PipeTransport created [res:%s]", data.dump().c_str());
				request->Accept(data);
